...
* `tmux_connection_state` - for panes in a managed [tmux control session](config/tmux_control.md), one of `connecting`, `syncing`, `connected`, `reconnecting`, or `disconnected`; otherwise `nil`. {{since('nightly', inline=True)}}
